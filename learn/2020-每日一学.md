...
(3) ES8 （ES2017）学习：async / await  , 是 generator 构造器函数的语法糖，function* foo() { ... yield} , async 表示异步, 相当于 *，写在 function 的前面 ;  await 表示等待， 相当于 yield ; async 会自动返回一个 Promise 对象，相当于 Promise.resolve('demo') ；await 后面通常情况下，会跟一个异步的操作；
